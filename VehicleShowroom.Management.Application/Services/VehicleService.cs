        ...
        public async Task<(bool Success, string ErrorMessage)> SaveOrUpdateAsync(VehicleViewModel model, IFormFile? fileUpload, string? oldImage = null)
        {
            try
            {
                ...
                if (model.VehicleId == 0)
                {
                    ...
                    vehicle.Price = (decimal)model.Price > 0 ? (decimal)model.Price : 0;
                    vehicle.Mileage = model.Mileage > 0 ? model.Mileage : 0;
                    vehicle.ManufactureYear = model.ManufactureYear > 0 ? model.ManufactureYear : 0;
